...
public interface FlowableFluidAccessor {
    ...
    int invoke_getNextTickDelay(World world, BlockPos pos, FluidState oldState, FluidState newState);

    @Invoker("tryFlow")
    void invoke_tryFlow(World world, BlockPos fluidPos, FluidState state);
}
