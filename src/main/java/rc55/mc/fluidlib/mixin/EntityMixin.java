...
public abstract class EntityMixin {
    ...
    public Stream<TagKey<Fluid>> fluidlib$appendDummyFluidTags(FluidState instance) {
        ...
        if (type == FluidSettings.EntityMovementType.WATER) {
            this.submergedFluidTag.add(FluidLibFluidTags.DUMMY_WATER_PHYSICS_TAG);
        } else if (type == FluidSettings.EntityMovementType.LAVA) {
            this.submergedFluidTag.add(FluidLibFluidTags.DUMMY_LAVA_PHYSICS_TAG);
        }
        if (!instance.isEmpty() && !instance.getFluid().getSettings().canEntityBreath()) {
            this.submergedFluidTag.add(FluidLibFluidTags.DUMMY_UNBREATHABLE_TAG);
        }
        return instance.streamTags();
    }

    @Inject(at = @At("HEAD"), method = "updateWaterState")
    public void fluidlib$initCustomFluidHeightMap(CallbackInfoReturnable<Boolean> cir) {
        this.fluidlib$fluidHeightMap.clear();
    }

    @ModifyReturnValue(method = "updateWaterState", at = @At("RETURN"))
    public boolean fluidlib$hookTouchingFluidCheck(boolean original) {
    ...
    public boolean fluidlib$fixPlayerSwimmingSpeed(Entity instance) {
        return this.isTouchingFluid(fluid -> fluid.getSettings().canSwim());
    }

    @Inject(at = @At("TAIL"), method = "baseTick")
    public void fluidlib$applyLavaLikeFluidEffects(CallbackInfo ci) {
    ...
    public boolean fluidlib$fixSprintingParticle(boolean original) {
        final Entity self = (Entity)(Object) this;
    ...
    public double getFluidHeight(TagKey<Fluid> tag) {
        if (tag == FluidTags.WATER) {
            return this.fluidHeight.getDouble(FluidLibFluidTags.DUMMY_WATER_PHYSICS_TAG);
        } else if (tag == FluidTags.LAVA) {
            return this.fluidHeight.getDouble(FluidLibFluidTags.DUMMY_LAVA_PHYSICS_TAG);
        }
    ...
    /**
     * @author redColmula55
     * @reason Make watery fluid(Movement type set to {@link rc55.mc.fluidlib.fluid.FluidSettings.EntityMovementType#WATER}) swimmable
     */
    @Overwrite
    public void updateSwimming() {
        ...
        if (self.isSwimming()) {
            ...
            self.setSwimming(self.isSprinting()
                    && this.submergedFluidTag.contains(FluidLibFluidTags.DUMMY_WATER_PHYSICS_TAG)
                    && !self.hasVehicle()
    ...
    @Unique
    private final Object2DoubleMap<Fluid> fluidlib$fluidHeightMap = new Object2DoubleArrayMap<>();
...
//            return bl2;
        ...
        list.forEach(fluid -> {
            this.fluidlib$fluidHeightMap.put(fluid, finalTouchedFluidHeight);

            final FluidSettings settings = fluid.getSettings();
            if (settings.getMovementType() == FluidSettings.EntityMovementType.WATER) {
                this.fluidHeight.put(FluidLibFluidTags.DUMMY_WATER_PHYSICS_TAG, finalTouchedFluidHeight);
                // Reduce fall damage in watery fluid
                self.onLanding();
            } else if (settings.getMovementType() == FluidSettings.EntityMovementType.LAVA) {
                this.fluidHeight.put(FluidLibFluidTags.DUMMY_LAVA_PHYSICS_TAG, finalTouchedFluidHeight);
            }

            if (fluid.isIn(FluidTags.WATER)) {
                this.fluidHeight.put(FluidTags.WATER, finalTouchedFluidHeight);
            }
            if (fluid.isIn(FluidTags.LAVA)) {
                this.fluidHeight.put(FluidTags.LAVA, finalTouchedFluidHeight);
            }
        });
    }
    ...
    public boolean isTouchingFluid(Predicate<Fluid> predicate) {
        return this.fluidlib$fluidHeightMap.object2DoubleEntrySet().stream().anyMatch(entry -> {
